...
def build_initial_guess(start, goal, bev_patch, N, dt, v_target=0.6):
    rects = []
    if bev_patch:
    ...
    xs, ys, ths, _ = heading_and_speed(pts, dt, v_target=v_target)
    nx, nu = 5, 2
    X_init = np.zeros((nx, N+1))
    U_init = np.zeros((nu, N))

    # 速度爬坡（保证会动）
    v_plan = np.linspace(0.0, v_target, N+1)

    for k in range(N+1):
        X_init[0, k] = xs[k]
        X_init[1, k] = ys[k]
        X_init[2, k] = ths[k]
        X_init[3, k] = max(v_plan[k], 0.05)
        X_init[4, k] = 0.0

    for k in range(N):
        dv  = v_plan[k+1] - v_plan[k]
        dth = ths[k+1] - ths[k]
        while dth >  np.pi: dth -= 2*np.pi
        while dth < -np.pi: dth += 2*np.pi
        U_init[0, k] = np.clip(dv/dt, -0.3, 0.3)
        U_init[1, k] = np.clip(dth,   -0.2, 0.2)

    # 动力学一致正向滚动
    X_sim = np.zeros_like(X_init)
    X_sim[:, 0] = np.array(start, dtype=float)
...
def build_and_solve(task: Dict[str, Any], bev_patch: Dict[str, Any] = None,
    ...
    if enable_no_backtrack:
        ...
        for k in range(N):
            ...
            g_list.append(forward_step + eps_back)
    ...
    dist_go = float(np.hypot(goal[0] - start[0], goal[1] - start[1]))
    T_total = N * dt
    # 到达目标所需的理论最小平均速度 + 20% 余量（也可改成常数 0.3）
    v_needed = (dist_go / max(T_total, 1e-6)) * 1.2
    v_max_geom = max(v_max_cfg, v_needed)

    for k in range(N):
        step = X[0:2, k+1] - X[0:2, k]
        move = ca.norm_2(step)
        g_list.append(v_max_geom * dt - move)  # >= 0
        lbg.append(0.0); ubg.append(ca.inf)
    ...
    # initial guess
    v_target_guess = max(0.35, v_needed)   # 或者直接用 v_max_geom*0.8
    X_init, U_init = build_initial_guess(start, goal, bev_patch, N=N, dt=dt, v_target=v_target_guess) 
    ...
    if debug_save_init:
        ...
        if bev_patch:
            ...
            for m in masks:
                if str(m.get("shape", "polygon")) != "polygon": continue
                pts = m.get("points", [])
                if pts:
    ...
    # IPOPT options（注意：不要放在上面的 if 内，否则缩进会错）
    opts = {
        "ipopt.print_level": 0,
        "print_time": 0,
        "ipopt.max_iter": 1200,
